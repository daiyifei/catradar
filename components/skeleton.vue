<template>
	<view v-if="loading" :style="{
	 ...
	 @touchmove.stop.prevent>
		<view v-for="(item, index) in RectNodes" :key="'rect'+index" :class="[animation ? 'skeleton-fade' : '']" :style="{
			width: item.width + 'px', 
		...
		<view v-for="(item, index) in circleNodes" :key="'circle'+index" :class="animation ? 'skeleton-fade' : ''" :style="{
			width: item.width + 'px', 
			height: item.height + 'px', 
			backgroundColor: elColor, 
		...
		<view v-for="(item, index) in filletNodes" :key="'fillet'+index" :class="animation ? 'skeleton-fade' : ''" :style="{
			width: item.width + 'px', 
			height: item.height + 'px', 
			backgroundColor: elColor, 
	...
	export default {
		...
		methods: {
			...
			selecterQueryInfo() {
				...
				// #endif
				query.selectAll('.skeleton').boundingClientRect().exec((res) => {
			...
			getRectEls() {
				...
				query = uni.createSelectorQuery()
				// #endif
				query.selectAll('.skeleton-rect').boundingClientRect().exec((res) => {
			...
			getFilletEls() {
				...
				query = uni.createSelectorQuery()
				// #endif
				query.selectAll('.skeleton-fillet').boundingClientRect().exec((res) => {
			...
			getCircleEls() {
				...
				query = uni.createSelectorQuery()
				// #endif
				query.selectAll('.skeleton-circle').boundingClientRect().exec((res) => {
